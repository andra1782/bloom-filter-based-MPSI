...
void benchmark(long repetitions, std::vector<long> number_of_parties_list, long set_size_clients, long set_size_server, int p_fraction, int false_positive_exponent=-30) {
    ...
    comp_csv << "Parties,Client,Leader Client,Server Authorize,Server Intersect,Judge\n";
    std::ofstream comm_csv("../data/communication.csv");
    comm_csv << "Parties,Client Sent,Client Received,Leader Client Sent,Leader Client Received,Server Sent,Server Received,Judge Sent,Judge Received\n";
    ...
    for (long t : number_of_parties_list) {
        ...
        std::vector<long> leader_client_computation_times;
        std::vector<long> server_authorize_times;
        std::vector<long> server_intersect_times;
        ...
        for (int i = 0; i < repetitions; ++i) {
            double client_computation_time = 0;
            double leader_client_computation_time = 0;
            double server_authorize_time = 0;
            double server_intersect_time = 0;
            double judge_computation_time = 0;
            ...
            std::vector<long> result = multiparty_psi(
                ...
                &leader_client_computation_time,
                &server_authorize_time,
                &server_intersect_time,
            ...
            leader_client_computation_times.push_back(static_cast<long>(leader_client_computation_time));
            server_authorize_times.push_back(static_cast<long>(server_authorize_time));
            server_intersect_times.push_back(static_cast<long>(server_intersect_time));
        ...
        double mean_leader_client_computation = sample_mean_computation(leader_client_computation_times);
        std_dev = sample_std_computation(leader_client_computation_times, mean_leader_client_computation);
        std::cout << "Leader Client computation time (ms): mean " << std::fixed << mean_leader_client_computation << ", std dev " << std_dev << std::endl;

        double mean_server_authorize = sample_mean_computation(server_authorize_times);
        std_dev = sample_std_computation(server_authorize_times, mean_server_authorize);
        std::cout << "Server authorize time (ms): mean " << std::fixed << mean_server_authorize << ", std dev " << std_dev << std::endl;

        double mean_server_intersect = sample_mean_computation(server_intersect_times);
        std_dev = sample_std_computation(server_intersect_times, mean_server_intersect);
        std::cout << "Server intersect time (ms): mean " << std::fixed << mean_server_intersect << ", std dev " << std_dev << std::endl;
        ...
        comp_csv << t << "," 
                << mean_client_computation << ","   
                << mean_leader_client_computation << "," 
                << mean_server_authorize << "," 
                << mean_server_intersect << "," 
        ...
        double total_comp_time = mean_leader_client_computation + mean_server_authorize + mean_server_intersect + mean_judge_computation;
        double bandwidth_total_bytes = (mean_server_sent + mean_server_received + mean_client_sent + mean_client_received + mean_leader_client_sent + mean_leader_client_received + mean_judge_sent + mean_judge_received) / 2;
        ...
        size_t messages = 4 + 2 * t + (t - 1) + 1; // 4 for authorize, 2 * t for OPRFs, t - 1 to send to leader, 1 leader to server
